...
int main() {
    ...
    // Loop para receber várias mensagens de Alice
    while (1) {
        // Receber a mensagem criptografada de Alice
        recv(new_sock, encryptedMessage, sizeof(encryptedMessage), 0);

        // Verificar se a mensagem é vazia, indicando o encerramento da comunicação
        if (strlen((const char *)encryptedMessage) == 0) {
            printf("Comunicação encerrada por Alice.\n");
            break;
        }
        ...
        // Exibir a mensagem criptografada enviada por Alice
        ...
        // Exibir a mensagem descriptografada recebida de Alice
        printf("Mensagem descriptografada recebida de Alice: %s\n", decryptedMessage);
