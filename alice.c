...
int main() {
    ...
    while (1) {
        ...
        printf("Digite a mensagem a ser enviada para Bob (ou 'sair' para encerrar a comunicação): ");
        fgets(message, 256, stdin);

        // Remover a nova linha da entrada
        message[strcspn(message, "\n")] = 0;

        // Verificar se a mensagem é para encerrar a comunicação
        if (strcmp(message, "sair") == 0) {
            // Se o usuário digitar "sair", encerrar a comunicação
            send(sockfd, encryptedMessage, 0, 0); // Enviar mensagem vazia para indicar o encerramento
            break;
        }
